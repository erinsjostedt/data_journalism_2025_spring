...
This PDF has multiple possible tables to extract. We're going to make four data frames: 
...
You will need to read in and clean up the data so that it can be used for analysis. By "clean" I mean the column headers must not contain spaces and they should have meaningful names, not "x1" or something similar.

The current registrations dataframe *MUST* include the county name, but should not include the columns under `Changes` and `Party Affiliation From`.
...
new_party_regs <- read_csv("data/new_reg_sept2024.csv")

glimpse(new_party_regs)
```

```{r}
# Removals

removals <- read_csv("data/removals_sept2024.csv")
...
changes <- read_csv("data/reg_change_sept2024.csv")
...
current_regs <- read_csv("data/active_sept2024.csv")

glimpse(current_regs)
...
past_new_regs <- read_csv("data/2022_new_reg.csv")

glimpse(past_new_regs)
...
past_removals <- read_csv("data/2022_removals.csv")

glimpse(past_removals)
...
past_changes <- read_csv("Data/2022_changes.csv")

glimpse(past_changes)
...
past_regs <- read_csv("data/2022_current_regs.csv")
...
A1. in 2024, voter registrants were 52.5% Democrats, 24% Republicans and 22% Unaffiliated. In 2024, it was 54.1% Democrats, 24% Republicans and 20.2% Unaffiliated. From 2022 to 2024, there was a 2% decrease in Democrats, no change in Republicans and a 0.9% increase in Unaffiliated. 

I think an interesting story that could come from these changes would be finding voters who changed their status from Democrats to something else, why they changed it, and what party they switched to.
...
current_regs |>
  filter(COUNTY == 'TOTAL') |>
  select(COUNTY, DEM, TOTAL) |>
  mutate(
    percent_dem_2024 = (DEM/TOTAL)*100
  )
...
past_regs |>
  ...
  select(COUNTY, DEM, TOTAL) |>
  mutate(
    percent_DEM_2022 = (DEM/TOTAL)*100
  )
...
current_regs |>
  ...
  mutate(
    percent_rep_2024 = (REP/TOTAL)*100
  )
...
past_regs |>
  ...
  select(COUNTY, REP, TOTAL) |>
  mutate(
    percent_rep_2022 = (REP/TOTAL)*100
  )
...
current_regs |>
  ...
  mutate(
    percent_unaf_2024 = (UNAF/TOTAL)*100
  )
...
past_regs |>
  ...
  select(COUNTY, UNAF, TOTAL) |>
  mutate(
    percent_unaf_2022 = (UNAF/TOTAL)*100
  )
...
Q2. Among the Maryland jurisdictions with the most registered voters, which one had the biggest percentage increase in unaffiliated voters between 2022 and 2024? How did that county vote for governor in 2022 and for president in 2020?

Contrast the partisan percentages of that county with the county that had the eighth-largest percentage increase in unaffiliated voters. You should display information from the data that shows the counties with the largest percentage increase first.
