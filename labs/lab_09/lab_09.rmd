...
**A1.** https://datawrapper.dwcdn.net/oepzs/2/ I think the story here is the extreme dip in election day voters in 2020, as well as the continued increase of mail-in and early voters since then. It would be interesting to see if the data for the 2024 election continues to have a trend of fewer election day voters.
...
md_votes_by_type <- md_votes_by_type |>
  clean_names() |>
  mutate(
    early_perct = early/total*100,
    elect_day_perct = election_day/total*100,
    mail_pect = mail/total*100
  )
...
md_votes_by_type2 <- md_votes_by_type |>
  select(year, early_perct, elect_day_perct, mail_pect)
...
write_csv(md_votes_by_type2, "data/md_votes_by_type2.csv", col_names = TRUE)
...
**A2.** https://datawrapper.dwcdn.net/piLTM/1/ I chose to track how many early voters were 65 years-old or older. What I'd like to learn from this data is how it compares to past years old elderly early voting, and whether or not there has been a change between this election and previous ones.
...
mdpops <- get_acs(geography = "county",
              variables = c(zippop = "B01003_001"),
              state = "MD",
              year = 2020)
```
