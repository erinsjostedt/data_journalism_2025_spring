...
**Both candidates spent the largest amount of money on advertising.**

```{r}

cleaned_md_senate_expenses <- read.csv("data/cleaned_md_senate_expenses.csv")

```

```{r}

cleaned_md_senate_expenses |>
...
**Overall, Hogan had less funding and spent less than Alsobrooks. Both candidates spent the most on advertising and salaries/staffing fees. The main difference between the two candidates is that Alsobrooks put more money into digital production, while Hogan spent more on events and travel. Based on this, it seems like Hogan probably had more in-person events than Alsobrooks.**
...
**Both Hogan and Alsobrooks spent the majority of their advertising fund during April and May of 2024. Alsobrooks' highest monthly amount spent on advertising is about 925,710 more on advertising than Hogan's highest monthly amount spend on advertising. She put expenses towards advertising more often than Hogan did, as demonstrated by how few months Hogan's campaign lists it as an expense..**

```{r}

cleaned_md_senate_expenses_two <- cleaned_md_senate_expenses |>
  mutate(ymd(date),
...
cleaned_md_sentate_exp_two |>
  filter(purpose_clean == 'Advertising', candidate == 'Alsobrooks') |>
...
cleaned_md_sentate_exp_two |>
  filter(purpose_clean == 'Advertising', candidate == 'Hogan') |>
  group_by(candidate, month) |>
  summarise(
    amount = sum(amount)
    ) |>
...
**The Alsobrooks campaign has the larger payroll/salary spending. I think Alsobrooks has 394 unique recipients for payroll, and Hogan has 18. If I'm wrong and supposed to be sorting by organization, then Alsobrooks has 2 unique recipients and Hogan has 1.**

```{r}

staff_expenses <- cleaned_md_senate_expenses |>
  mutate(
    payroll_fees = case_when(
      purpose_clean == 'STAFFING SERVICES' | purpose_clean == 'Salary' | purpose_clean == 'Payroll' ~ "Payroll"
      )
  ) |>
  filter(payroll_fees == 'Payroll') |>
  group_by(candidate, payroll_fees) |>
  summarise(
    amount = sum(amount)
    ) |>
  arrange(desc(amount))
...
cleaned_md_senate_expenses |>
  filter(purpose_clean == 'STAFFING SERVICES' | purpose_clean == 'Salary' | purpose_clean == 'Payroll') |>
  group_by(candidate) |>
...
**I think it would be interesting to look into why so much campaign money goes into advertising and how the amount spent on advertising has changed over the years.**
