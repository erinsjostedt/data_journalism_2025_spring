...
```{r}
early_voting_summary <- clean_early_voting_turnout |>
  group_by(county) |>
  summarise(
    total_early_votes = sum(total, na.rm = TRUE),
    eligible_voters = sum(eligible_actives, na.rm = TRUE)
  ) |>
  mutate(
    turnout_percent = (total_early_votes / eligible_voters) * 100
  ) |>
  arrange(desc(turnout_percent))

early_voting_summary
