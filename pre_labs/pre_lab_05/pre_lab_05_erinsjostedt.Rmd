...
```{r}
county_voters_2016 <- read_csv("county_voters_2016.csv")
county_voters_2018 <- read_csv("county_voters_2018.csv")
county_voters_2020 <- read_csv("county_voters_2020.csv")
county_voters_2022 <- read_csv("county_voters_2022.csv")
county_voters_2024 <- read_csv("county_voters_2024.csv")
```
...
```{r}
maryland_population <- read_csv('maryland_population_2020.csv')
```
...
**Task** Run the following code to join Maryland population data and the 2020 voter registration data together using the common column county as the key. How many rows are there now? How many *should* there be? **There are 25 rows right now but there should be 24**
...
**Task** Create a new dataframe with population and voter data joined together. What does the output message below say about the join? **This is doing a left join by the common column COUNTY. All rows from maryland_population will be kept, and voter data from county_voters_2020 will be added where there is a matching county. If some counties in maryland_population don’t exist in county_voters_2020, their voter data will appear as NA.**
...
```{r}
maryland_demographics <- read_csv('maryland_demographics.csv')
```
...
**Task** Using mutate, let's find the county with the highest 18+ Black population as a percentage of all population 18 and over and also the percentage of Democrats in that county. Put your answers below. **Prince George's county, 78.2% democratic.**
...
**Task** Now let's find the county with the highest 18+ Asian population as a percentage of all population 18 and over and also the percentage of Democrats in that county. How do these results compare to the previous task? Put your answers below. **Howard County, 52.377% democratic.** 
...
**Task** Run the following code to load the Zip Code Tabulation Area data for Maryland. What's the datatype of the ZCTA5N? column? **<dbl>**
...
**Task** Run the following code to change the datatype of ZCTA5N from numeric to character. What's the datatype of ZCTA5N now? **will be a character now.**
