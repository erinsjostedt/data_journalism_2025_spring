...
**Task** Run the following codeblock to reload the data, using every row to set the data types. Does it show any parsing errors when you run? Answer below **No errors found**
...
**Task** Run the following codeblock to group by mail votes, count the number of precinct results, then sort from highest to lowest on count. How many results are there where mail is NA? What's the implication there? Answer below. **There are 402345 results where mail is NA. This implies that there could have been an error in data collection, resulting in many missing values. **:
...
**Task** Run the following codeblock to find the number of registered voters grouped by voter status reason, using summarise() to count. Are any of these worthy of exploration to you? Why or why not? Answer below. **I think it is work exploring confirmation not returned becasue the count is relatively high and I would like to find a reason why they are not being returned. This could be because of issues in voter engagement or communication, etc. **
...
**Duplicate entries, missing data, missing clear identifiers**
...
**Converts all column names to lowercase, replaces spaces and special characters with underscores ensuring that column names are standardized, adds a prefix (x) to column names that start with numbers to make them readable in.**
...
**Task** Run the following codeblock to attempt to add up the amount of all contributions. What does the error say when you run this code? What do you think the error means? Answer below. **The error says there is an error in the amount column. It is being recognized as a character instead of a numerical column.**
...
**Task** Run the following codeblock to check for duplicate rows using get_dupes(). How many are there? What is the donor name? Answer below. **Derrick Hamilton. There is 2 of them.**
...
**Task** Run the following codeblock to use distinct() to get rid of duplicate rows. How many rows does the new dataframe have? Answer below. Add a description of what this code does to your reference notebook. **The new dataframe has 13 rows**
...
**Task** Run the following codeblock to use str_sub() to convert the ZIP codes that have nine digits to five digits, standardizing the field. Look at the difference in the result - what changed? **All zip codes now have 5 digits**
...
**Task** Run the following codeblock to use str_tot_title() to standarize capitalization in the "city" field. How many mispellings of Conowingo remain after running this code? Answer below. Add a description of what this code does to your reference notebook. **I now see two misspellings (Conowing, Conowingoo)**
...
**Task** Run the following codeblock to use case_when() to fix misspellings of Conowingo in the "city" field. How many mispellings of Conowingo remain after running this code? Answer below. **one**
...
**Task** Run the following codeblock to use case_when() to fix misspellings of Conowingo in the "city" field using both the exact match method and the str_detect() method. How many mispellings of Conowingo remain after running this code? Answer below. Add a description of what this code does to your reference notebook. **Zero misspellings**
