...
```{r}
voters_by_county <- read_csv("tabula-md_voters_by_county.csv")
...
```{r, echo=FALSE}
knitr::include_graphics(rep("benzie_1.png"))
```
...
```{r, echo=FALSE}
knitr::include_graphics(rep("benzie_2.png"))
```
...
```{r, echo=FALSE}
knitr::include_graphics(rep("benzie_3.png"))
```
...
```{r, echo=FALSE}
knitr::include_graphics(rep("benzie_4.png"))
```
...
```{r}
benzie_2024 <- read_csv("tabula-benzie_aug.csv")
...
```{r}
benzie_2024 <- read_csv("tabula-benzie_aug.csv", col_names = FALSE)
...
**Task** Run the codeblock below. What changed, and what issues or problems remain? **We have headers we can comprehend and disect better rather than simply X1-X30**
