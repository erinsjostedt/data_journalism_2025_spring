...
```{r}
dem_cands <- read_csv("md_dem_cands.csv")
```
...
```{r, echo=FALSE}
knitr::include_graphics(rep("datawrapper3.png"))
```
...
```{r, echo=FALSE}
knitr::include_graphics("datawrapper4.png")
```
...
```{r, echo=FALSE}
knitr::include_graphics("madatawrapper5.png")
```
...
```{r, echo=FALSE}
knitr::include_graphics("datawrapper6.png")
```
...
**https://datawrapper.dwcdn.net/684oj/1/** Copy the url of the graphic you published here. It will begin with <https://datawrapper.dwcdn.net/>. Don't copy the browser's current url.
...
```{r}
md_gov_county <- read_csv("md_gov_county.csv")
```
...
write_csv(md_gov_county, "md_gov_county.csv")
```
...
```{r, echo=FALSE}
knitr::include_graphics("datawrapper7.png")
```
...
```{r, echo=FALSE}
knitr::include_graphics("datawrapper8.png")
```
...
**Task** Look for "Hide regions without data" under Appearance, and click the slider icon to enable that feature. You should see a map zoomed into Maryland with some counties in various colors. **The map is now only zoomed in one the state of MD. We only see MD data** Describe the changes from the previous map
...
**Task** Click the "Customize tooltips" button so it expands down. Change {{ fips_code }} to {{ county }} and {{ pctmoore }} to {{ FORMAT(pctmoore, "00.0%")}} **This map now has labels and the counties are visible by zip code and name on the interactive tooltip.** Describe the changes from the previous map
...
**Task** Click the "Proceed" button until you get to the "Publish & Embed" tab, then click "Publish Now". Copy the published URL (like you did for the chart above) and paste it below. **https://datawrapper.dwcdn.net/YFlW2/1/** Put the URL of your map here.
