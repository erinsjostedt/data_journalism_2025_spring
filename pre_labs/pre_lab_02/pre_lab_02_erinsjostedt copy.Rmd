...
**The code is selecting the specific columns from the dataset that it wants to include and arranging them in descending order. * 100.  **
...
**We created a new column, percent_election_day which **
...
**Yes, this answer is more useful if you are seeking the records with the lowest election day votes. They are now arranged in ascending order.**
...
**str_to_lower() - Converts all characters in a string to lowercase.
str_to_title() - Capitalizes the first letter of each word in a string, useful for standardizing titles or names.**
...
**The mutate statement is creating a new column called amount_category based on the values in the amount column. The case_when function checks the value of amount for each row and assigns a category based on the specified ranges. This helps group the data into meaningful categories based on contribution amount.**
...
**This is important because that way we are not altering the original state of the data but we can still hold a place for the results.**
...
**Answer: We changed it to a proper date format so that R will be able to perform date-related functions instead of just recogizing it as a character.**
...
**The new column for month that we created shows each expenditure by month and year. The floor month function rounds down specific dates to the first day of the month, July 2022 days are all 2022-07-01 for example.**
