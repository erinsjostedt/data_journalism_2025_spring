...
**I think is creating a new column (variable) for the amount of votes cast per candidate on election day (as opposed to early voting or mail in). It is doing this by taking the number of votes made on election day and dividing it by the total number of votes.**
...
**The `arrange` function reordered the table by descending percentage of votes made on election day.**
...
**It might be more useful depending on the story you're writing. For example, if you're writing about election results and splitting up votes based on when they were cast, it might be beneficial to make it a new variable and organize it by that.**
...
**`str_to_lower` converts the text into lowercase. `str_to_sentence` converts the text so that the first letter of every sentence is capitalized and the rest is lowercase.**
...
**I think the mutate statement is creating new categories based on the amount donated. Each row gets assigned a label, and the assignment of these labels are determined by what numerical range the amount donated fits in.**
...
**head(selected_prince_geourges) limits the head to show only the selected data and not the data for all of PG county.**
...
**The code changes the cokumn type to `<date>` and reformats the individual dates into a readable format.**
...
**I think the values in the `month` column are showing the total amount of expenses from each month, organized by descending amount.**
