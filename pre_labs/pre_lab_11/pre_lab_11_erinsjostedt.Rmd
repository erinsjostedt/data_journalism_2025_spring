...
**Task** Run the following code in the gray-colored codeblock below to extract all tables in the HTML and save it as a dataframe. What do you notice about the columns? **the column headers are long**
...
**Task** Run the following code to extract the table we want, clean up the names and remove the total row. Look at the rename() function - what do you think the numbers 2 through 6 are referring to? **the numbers 2-6 are referring to the column numbers**
...
**Task** Run the following code in the gray-colored codeblock below to find the <li> tags. What does the output suggest about how many <ul> tags there are in the HTML source? **the output suggests that there are a lot of <ul> tags since we have a lot of <li> tags. This probably means the data has more things than we need because it is pulling from the entire page not just what we need.**
...
**Task** Run the following code in the gray-colored codeblock below to find the text inside <li> tags. Describe the output: **Some of these things outputted are committee names which we do want, but a lot of them are unrelated items like nav links and the site map. It’s not clean or filtered yet, just raw text from every list item (which to be fair is what we asked the computer for).**
...
### Task 11: Turn the contents of the <li> and <a> tags into a dataframe **Task** Run the following code in the gray-colored codeblock below to make a dataframe. Describe the output: **The output is a dataset with two columns: name, which contains the text from each link, and url, which contains the href link. It includes both relevant and irrelevant links, so we'll need to do some filtering as a good data journo does.**
...
And now you have the committee names and urls. **boom.. mic drop**
