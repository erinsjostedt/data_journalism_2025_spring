...
```{r echo=FALSE}
census_api_key("16f215cb8e93e93f49f1866e5ec02a4366d65550", install=TRUE)
```
...
**Task** Run the following codeblock to find the Maryland county with the highest median household income and put the answer below. **Howard County, MD**
...
```{r}
md_contribs <- read_rds("pre_lab_07.rds")
```
...
**21532, 21851, 21822, 21531, 21632, 21216, 21817, 21634**
...
**Task** Arrange md_zips_with_acs by estimate showing the highest median household income first. What questions do you have about these top zip codes? **I would like to know what these people's occupations are, are there geographic patterns, are there outliars in here?**
```{r}
md_zips_with_acs <- md_zips_with_acs |> arrange(desc(estimate))
md_zips_with_acs
```
