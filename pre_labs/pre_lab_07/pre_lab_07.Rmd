...
```{r}
p10 |> select(GEOID, NAME, value) |>
  rename(Population2010=value) -> p10

p00 |> select(GEOID, NAME, value) |>
...
**Answer** The state with the largest percentage of growth was Nevada with a 35% increase in population.

```{r}
alldata |> mutate(change = ((Population2010-Population2000)/Population2000)*100) |>
  arrange(desc(change))
```
...
**Answer** Howard County has the highest median household income of all of the Maryland counties.
...
**Answer** The difference between Calvert and Montgomery county is not very significant. Their bars overlap, so the difference between them is well within the margin of error.

Is the difference between Howard and everyone else significant? 
**Answer** Howard county is significantly different from the other Maryland counties. It has a higher ACS estimate than any other county and its margin of error does not overlap with any other ones.
...
**Answer** No counties saw a decrease in their median incomes from 2010 to 2022. 
...
**Answer** None of the top 20 zip codes have estimated median household incomes of less than $50,000.
...
**Answer** Why do so many of the households share similar zip codes ("2081X")? What area are these houses in? Are they all in the same neighborhood? How do their estimated incomes compare to the rest of the county, and is the difference significant?

```{r}

md_zips_with_acs |>
  group_by(zip5, estimate) |>
  summarise(total) |>
  arrange(desc(total))
