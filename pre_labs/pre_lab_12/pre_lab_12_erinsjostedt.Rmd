...
**Task** Run the following code to gather the text for the first 10 press releases. **I didn't end up doing the WayBack becasue I could not find the url in here. I do understand what it is asking of me though nad I am glad you and I were able to figure out the diagnosis for why it was not working! Hopefully that is sufficient.**
...
**Task** Run the following code and describe what the `unique` function does to the original list of words. **It removes any duplicates**
...
**Task** Run the following code and describe the output of using the `unnest_tokens` function. **it brings each line of text into its own row under the column word. It separates their values and makes them each individual tokens with no punctuation or upper cases.**
...
**Task** Run the following code and describe what the resulting graphic shows. Is it interesting? **It gives us the 25 most commonly used words in the press release or dataset and how often each one shows up. This COULD be interesting but right now not really because the top words are boring: "the, and" etc. **
...
**Task** Run the following code and describe the results. Is it more interesting than before? **Yes this actually gives us something we can use, not just filler words. Based on this we know he talks a lot about health, support, funding, taking action, etc. This could narrow a search for important topics to cover on Cardin.**
...
**Task** Run the following code and describe the results. **We get 2 dataframes showing the top 10 most commonly used words on this site that year. In 2022 the top word is health, in 2024 the top word was baltimore.**
...
**Task** Run the following code and describe the results. **This result gives us a list of bigrams or word combinations most frequently used. Even just 2 words gives us a lot more context than the simple one words. Amount included was the top bigram for 2022.**
...
**Task** Run the following code and describe the results. **Same result here for 2024, we have more context! So we now know that healthcare was the most frequently used bigram in 2024 on this site.**
...
**Task** Run the following code and describe the differences between the two results. Which one might be more useful for the press release data? **I think the lemmatizing version is more helpful beecause it gives us less of a needle in a haystack to search for -- we now know that when we search any of the variations of drive we will get results, not picky.**
...
**Task** Run the following code and describe the differences between the two results. Which one is more useful? **I think in this example the lemmatized list is still more useful because it doesn't switch between a bunch of different forms of the same word, giving a clearer picture of the most important points**
...
**Task** Run the following code and describe the results. Do any of the sentiments seem incorrect or counter-intuitive? **This result gives us the most positive and negative words between the unique words datasets. I think few and far between words can be misleading like "issue." Issue could also just mean to give someone something officially with documentation. Additionally the word "right" could mean right-wing extremism or something and we dont get to see that. Overall though this does do a pretty consistently good job at being accurate.**
