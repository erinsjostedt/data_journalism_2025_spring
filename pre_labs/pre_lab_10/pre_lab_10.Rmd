...
**Task** Run the following code to load data. Describe what you see in the answer space below. What columns exist in this data? **PRECINCT_ID, PRECINCT_NAME, CONGRESS, LEGIS, COUNCIL, SCHOOL, POLLING_ID, Shape_Length, Shape_Area, Shape are the columns that are in this data.**
...
**Task** Run the following code. Describe the output in the space below: what kind of information does it contain? **This file maps each precinct to its respective legislative, council, and school board districts, shows where people vote, and provides the geometry for creating precinct-level maps.**
...
**Task** Run the following code. Describe the output in the space below. **The output is a plot of the precincts in PG county based on voting precint boundaries**
...
**Task** Run the following code. Describe the output in the space below. **It just tells me to enable caching**
...
**Task** Run the following code. Describe the output in the space below. **The code loads a dataset of all U.S. counties and uses glimpse() to show its structure, including columns like county name, land area, and geographic shape.**
...
**Task** Run the following code. Describe the output in the space below. **Gives us the full U.S. counties dataset including only counties in Maryland, which have a state FIPS code of 24**
...
**Task** Run the following code. Describe the output in the space below. **This code creates a simple map of Maryland’s counties using ggplot2**
...
**Task** Run the following code. Describe the output in the space below. **The plot layers Prince George’s County boundary and precincts together. You can see the county outline with precincts overlaid on it.**
...
**Task** Run the following code. Describe the output in the space below. **This loads the turnout data for Prince George’s County, showing polling, early voting, absentee, provisional, and eligible voters for each precinct.**
...
**Task** Run the following code. Describe the output in the space below. **This joins the voting data to each precinct shape based on precinct IDs.**
...
**Task** Run the following code. Describe the output in the space below. **The map colors in precincts based on the number of eligible voters. White areas represent precincts with no eligible voters.**
...
**Task** Run the following code. Describe the output in the space below. **Montgomery County has the highest number of eligible voters, followed by Prince George's County and Baltimore County.**
...
**Task** Run the following code. Describe the output in the space below. **Loads total population estimates for each of the Maryland counties from the ACS.**
...
**Task** Run the following code. Describe the output in the space below. **Loads total Maryland county population data from the 2022 ACS and adds a geometry column with multipolygon shapes for mapping each county.**
...
**Task** Run the following code. Describe the output in the space below. **Displays the md_county_population and voters_by_county  side-by-side. Shows that county names are formatted differently in the datasets and need to be cleaned to join properly.**
...
**Task** Run the following code. Describe the output in the space below. **Cleans `md_county_population` and renames the "NAME" column to "County," removes "County" and ", Maryland," keeps only the County, population, and geometry columns.**
...
**Task** Run the following code. Describe the output in the space below. **Joins the cleaned `md_county_population` with `voters_by_county`. Some counties do not match all the way still.**
...
**Task** Run the following code. Describe the output in the space below. **Fixes county names using `if_else` to rename St. Mary’s and Baltimore to match the voter dataset.**
...
**Task** Run the following code. Describe the output in the space below. **Successfully joins the corrected county names and calculates the number of voters per 10,000 people in each county. Sorts counties from highest to lowest voter-per-population ratio.**
...
**Task** Run the following code. Describe the output in the space below. **Creates a choropleth map showing the number of voters per 10,000 residents across Maryland counties. Some differences between counties are visible.**
...
**Task** Run the following code. Describe the output in the space below. **Applies the magma color scale to the map, making the highest-ranking counties stand out more clearly.**
...
**Task** Run the following code. Describe the output in the space below. What regional patterns do you see? **Applies a logarithmic color scale to better show differences between counties.  I see central Maryland counties like Frederick, Charles, and Howard have higher voter registration rates; rural counties show lower rates.**
...
**Task** Run the following code. Describe the output in the space below. **Adds a new column calculating unaffiliated voters per 10,000 residents in each county.**
...
**Task** Run the following code. Describe the output in the space below. What regional patterns do you see, especially on the ends of the scale? **Creates a map of unaffiliated voters per 10,000 residents.  I see higher unaffiliated rates are seen in suburban counties like Howard and Frederick, while rural counties show lower unaffiliated rates.**
